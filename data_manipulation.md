...
Which data frame, and which variable columns are you selecting can list
the columns, or do this through that (:), the - will do everything
except what you - GROUP = group renames that variable by itself (will
also put it first), the everything says keep everything else
...
Which rows do I need to keep? Filter and keep all the litters where x, y
(something inside your data set)
...
``` r
## rearrange so that they are in order of pups born alive 3 -> 4 -> 5 etc 
